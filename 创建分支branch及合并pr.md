# 1. 从 master 新建 dev 分支
	...
	1. Branch‑Create → 起名，勾选 Checkout → Create
		（1. 点击 `Remote` → `Push`
		（2. Source Branches 选择 **dev**
		（3. Remote 保持 `origin`，所有复选框不勾选
		（4. 点 Push

	推送成功后，GitHub 页面就从 `1 Branch` → **2 Branch**，网页上就能看到 dev 分支。
...
# 5. 本地 fetch，merge origin/master 同步到本机 master
	...
	2. 菜单 `Remote` → `Fetch from origin`，拉取远程最新信息
	3. 菜单 `Merge` → `Local branch` 选 `origin/master`，执行合并
